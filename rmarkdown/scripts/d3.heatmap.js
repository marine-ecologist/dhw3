document.addEventListener("DOMContentLoaded", function() {
  ...
  // Dynamically create header and select element ABOVE heatmap container
  ...
  selectElement.setAttribute('placeholder', 'Search by reef name or GBRMPA ID');
  ...
  function drawHeatmap(meta, matrix, yearColumns, labelType = "lat") {
  ...
  drawHeatmap(meta, matrix, yearColumns);
  ...
  const select = new TomSelect('#reefSelect', {
    ...
    onChange: function(selectedIds) {
      ...
      if (!isSubset) {
        drawHeatmap(meta, matrix, yearColumns, 'lat');
        return;
      }
      ...
      drawHeatmap(filteredMeta, filteredMatrix, yearColumns, 'id');
    }
  });
 });
});